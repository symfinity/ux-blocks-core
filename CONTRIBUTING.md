# Contributing to UX Blocks Core

> **Read-only mirror.** This repository (`symfinity/ux-blocks-core`) is published from the Symfinity product monorepo (`packages/ux-blocks-core/`). **Do not expect pull requests to be merged here.**
...
Open an [issue](https://github.com/symfinity/ux-blocks-core/issues) on this repository (preferred for discoverability from Packagist and GitHub).
...
Open a pull request on the **product monorepo** (`packages/ux-blocks-core/`) on a `feature/{spec-id}-{slug}` branch. Do not rely on merging on this mirror.
